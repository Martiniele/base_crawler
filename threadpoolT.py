...
class Pool(object):
    ...
    def __init__(self, size):
        self.input_queue = Queue.Queue(maxsize=size)
        self.output_queue = Queue.Queue(maxsize=size)
        self.error_queue = Queue.Queue(maxsize=size)
        self.tasks = [
...
def main():
    """ main """
    pool = Pool(size=4)
    pool.add_tasks([(test, (i,)) for i in range(10)])
    pool.run()
